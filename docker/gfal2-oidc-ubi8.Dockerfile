FROM registry.access.redhat.com/ubi8/ubi-minimal AS oidc-builder
...
RUN \
    ...
    microdnf --enablerepo=powertools install -y \
    libcurl-devel \
    libsodium-devel \
    libmicrohttpd-devel \
    libsecret-devel \
...
FROM registry.access.redhat.com/ubi8/ubi-minimal AS gfal-builder
COPY ./assets/conf/dmc-el8.repo /etc/yum.repos.d/
RUN \
    rpm -ivh http://mirror.centos.org/centos/8-stream/BaseOS/x86_64/os/Packages/centos-gpg-keys-8-6.el8.noarch.rpm && \
    rpm -ivh http://mirror.centos.org/centos/8-stream/BaseOS/x86_64/os/Packages/centos-stream-repos-8-6.el8.noarch.rpm && \
    rpm -ivh https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm && \
...
COPY --from=gfal-builder \
 /usr/local/gfal2-util/lib/python3.6/site-packages/gfal2_util \
 /usr/local/lib/python3.6/site-packages/gfal2_util/
COPY --from=gfal-builder \
 /usr/local/gfal2-util/lib/python3.6/site-packages/gfal2_util-1.8.0-py3.6.egg-info \
